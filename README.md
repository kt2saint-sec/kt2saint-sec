
# Hello World :) 👋

  ## 🎯 About Me - kt2saint-sec

  Im Karl T. Career sales professional/entrepreneur transitioning into transitioning into the Cybersecurity field with hands-on experience in security hardening, networking, compliance automation, and threat detection. Currently building practical expertise through real-world security projects and pursuing industry certifications.

  ## 🛠️ Technical Skills

  **Security & Compliance**
  - CIS Benchmarks & security hardening
  - NIST frameworks & compliance auditing
  - Vulnerability assessment & risk analysis
  - Security policy implementation

  **Automation & Scripting**
  - Bash scripting for security automation
  - Ansible for infrastructure as code
  - Python for security tools
  - Linux system administration

  **Security Tools & Technologies**
  - UFW (Uncomplicated Firewall)/iptables
  - AppArmor & SELinux
  - OpenSCAP for compliance scanning
  - fail2ban, AIDE, rkhunter, Lynis
  - Wireshark & network analysis
  - Docker for security testing environments

  **Systems & Platforms**
  - Ubuntu/Debian Linux hardening
  - Kernel security & tuning
  - Server security architecture
  - Virtual machine security testing

  ## 🎓 Certifications

  🎯 **In Progress**
  - CompTIA Security+ (Target: Q1 2026)
  - CompTIA Network+ (Target: Q2 2026)
   
  ## 🚀 Featured Projects

  ### 🔒 [sec-levels](https://github.com/kt2saint-sec/sec-levels)
  Production-ready CIS Benchmark hardening automation toolkit for Ubuntu 24.04 LTS. Achieves 93.6% automation coverage (280/313 CIS controls).

  **Technologies**: Bash, Ansible, OpenSCAP, UFW, AppArmor, fail2ban
  **Skills Demonstrated**: Security hardening, compliance frameworks, infrastructure automation, Linux administration, comprehensive testing methodology

  ---

  ## 📚 Currently Learning

  - Advanced threat detection and SIEM analysis
  - Network security fundamentals & packet analysis
  - Security automation and DevSecOps practices
  - Cloud security (AWS/Azure)

  ## 📫 Connect on X --> @kt2saint-sec

  📍 GitHub: [@kt2saint-sec](https://github.com/kt2saint-sec)
